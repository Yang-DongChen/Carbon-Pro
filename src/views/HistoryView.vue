  ...
  <div class="history-page">
    ...
    <main class="content">
      ...
      <div v-else class="timeline-container">
        <div
          v-for="(list, date) in groupedRecords"
          :key="date"
          class="date-group"
        >
          ...
          <div class="record-list">
            <div v-for="item in list" :key="item.timestamp" class="record-item">
              <div
                class="icon-box"
                :style="{ background: typeConfig[item.type].color }"
              >
                <el-icon>
              ...
              <div class="info-box">
                ...
                <div class="item-details">
                  <span class="detail-tag"
                    >录入: <strong>{{ item.value }}</strong></span
                  >
                  <span class="detail-divider">/</span>
                  <span class="detail-tag"
                    >减排: <strong>{{ item.co2 }} kg</strong></span
                  >
                </div>
              </div>
              ...
              <div class="action-box">
                <span class="time-tag"
                  >{{ new Date(item.timestamp).getHours() }}:{{
                    String(new Date(item.timestamp).getMinutes()).padStart(
                      2,
                      "0",
                    )
                  }}</span
