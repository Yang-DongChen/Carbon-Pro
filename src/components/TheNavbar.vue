...
const handleFileChange = (e) => {
  ...
  if (file.size > 2 * 1024 * 1024)
    return ElMessage.warning("图片大小不能超过 2MB");
...
const saveAvatar = () => {
  ...
  userStore.updateProfile({ avatar: tempAvatar.value });
  showAvatarDialog.value = false;
  ElMessage.success("头像更新成功");
};
const saveProfile = () => {
  if (!profileForm.name.trim()) return ElMessage.warning("昵称不能为空");
      ...
      <div class="nav-content">
        ...
        <div class="nav-links desktop-only">
          ...
          <router-link to="/history" class="nav-item" active-class="active"
            >历史</router-link
          >
          <router-link to="/rewards" class="nav-item" active-class="active"
            >权益</router-link
          >
    ...
    >
      ...
      <template #footer
        ><div class="dialog-footer">
          ...
          ><el-button type="danger" @click="savePassword">修改</el-button>
        </div></template
      >
    </el-dialog>
  </div>
